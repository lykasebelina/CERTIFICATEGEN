...
export function positionSignaturesAdvanced(
  ...
  if (count === 1) {
    ...
    if (isLandscape) {
      ...
      signatures[0].y = bottomY  - rowGap * 1.8;
      signatures[1].y = bottomY ;
    }
  }
    ...
    if (isLandscape) {
      // L, R on row 1 (No Indent)
      signatures[0].x = leftX * 1 - 20;
      signatures[1].x = rightX * 1 + 20;
      signatures[0].y = signatures[1].y = bottomY * 1.02 - rowGap * 2.2;

      // L, R on row 2 (No Indent)
      signatures[2].x = leftX * 1 - 20;  
      signatures[3].x = rightX * 1 + 20; 
      signatures[2].y = signatures[3].y = bottomY * 1.05;
    } else {
      // Portrait: Top row is 2 wide, bottom row is 2 wide
      signatures[0].x = leftX;
      signatures[1].x = rightX;
      signatures[0].y = signatures[1].y = bottomY - rowGap * 1.6;

      // 🔑 CHANGE MADE HERE: Removed indentation for the bottom row
      signatures[2].x = leftX;  // Changed from indentLeft * 1 - 22
      signatures[3].x = rightX; // Changed from indentRight * 1 + 22
